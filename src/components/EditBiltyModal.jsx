...
const EditBiltyModal = ({ isOpen, onClose, bill, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl animate-in zoom-in duration-300">
        <div className="sticky top-0 bg-white border-b p-6 flex justify-between items-center z-10 uppercase font-black">
          <h2 className="text-xl md:text-2xl text-slate-800 tracking-tighter">Edit Bilty: {bill.LRNO}</h2>
            ...
            {Object.keys(formData).map((key) => (
              <div key={key} className="space-y-1">
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-blue-600 outline-none bg-slate-50 font-bold"
                />
              </div>
            ))}
          </div>
          <div className="flex justify-end gap-4 pt-6 border-t font-black uppercase text-xs">
            <button type="button" onClick={onClose} className="px-6 py-3 text-slate-600">Cancel</button>
