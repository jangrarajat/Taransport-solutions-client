...
const MaintenanceModal = ({ isOpen, onClose, bill, onUpdate, showNotification }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/50 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md   shadow-2xl p-6 animate-in zoom-in duration-200">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input required type="number" value={amount} onChange={(e) => setAmount(e.target.value)}
                            className="w-full border   px-4 py-3 bg-slate-50 dark:bg-slate-800 dark:text-white outline-none focus:border-orange-400" placeholder="e.g. 2000" />
                    </div>
                    <div>
                        ...
                        <input required type="text" value={remark} onChange={(e) => setRemark(e.target.value)}
                            className="w-full border   px-4 py-3 bg-slate-50 dark:bg-slate-800 dark:text-white outline-none focus:border-orange-400" placeholder="Tyre change, Oil, etc." />
                    </div>
                    <button disabled={loading}
                        className="w-full bg-orange-500 text-white py-4   font-black uppercase text-xs tracking-widest shadow-lg shadow-orange-100 dark:shadow-orange-900/50 active:scale-95 transition-all flex items-center justify-center gap-2">
