...
const DriverLedger = ({ driverId, driverName, onBack, showNotification }) => {
  ...
  const handleAddTransaction = async (data) => {
    try {
      await axios.post(`${backendUrl}/api/driver-transactions/transaction`, { driverId, ...data }, { withCredentials: true });
  ...
  const exportToExcel = () => {
    const exportData = transactions.map(t => ({
      ...
      Type: t.type === 'given' ? 'Given (I paid)' : 'Received (I got)',
    ...
    const summaryData = [
      { Item: 'Driver', Value: driverName },
      { Item: 'Period', Value: `${startDate || 'Start'} to ${endDate || 'End'}` },
      { Item: 'Opening Balance', Value: `₹${openingBalance.toLocaleString('en-IN')}` },
      { Item: 'Total Given (I paid)', Value: `₹${totalGiven.toLocaleString('en-IN')}` },
      { Item: 'Total Received (I got)', Value: `₹${totalReceived.toLocaleString('en-IN')}` },
      { Item: 'Net Change', Value: `₹${netChange.toLocaleString('en-IN')}` },
      { Item: 'Closing Balance', Value: `₹${closingBalance.toLocaleString('en-IN')}` }
    ];
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <div class="summary">
        ...
        <p><strong>Total Given (I paid):</strong> ₹${totalGiven.toLocaleString('en-IN')}</p>
        <p><strong>Total Received (I got):</strong> ₹${totalReceived.toLocaleString('en-IN')}</p>
        ...
        <tbody>
          ${transactions.map(t => `
            <tr>
              ...
              <td class="${t.type}">${t.type === 'given' ? 'Given (I paid)' : 'Received (I got)'}</td>
              <td>${t.description || '-'}</td>
    ...
    <div className="space-y-4 dark:text-white pb-20">
      ...
      <div className="flex flex-wrap items-center gap-2 bg-white dark:bg-slate-800 p-3 shadow-sm border border-slate-200 dark:border-slate-700">
        ...
        <button 
          ...
          className="p-2 bg-gray-200 dark:bg-slate-600 hover:bg-gray-300 dark:hover:bg-slate-500" 
          title="Reset"
        >
      ...
      {!loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Given (I paid)</p>
            <p className="text-xl font-black text-red-600 dark:text-red-400">₹{totalGiven.toLocaleString('en-IN')}</p>
          </div>
          <div className="bg-white dark:bg-slate-800 p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Received (I got)</p>
      ...
      {!loading && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded p-3 text-center font-bold text-sm">
          <span className="text-slate-700 dark:text-slate-300">
            Net Change (Received – Given):{' '}
            <span className={`${netChange >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {transactions.map(t => (
                  <tr 
                    key={t._id} 
                    className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 ${selectedIds.includes(t._id) ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''}`}
                  >
                    ...
                    <td className="px-4 py-2">
                      <span className={`px-2 py-1 rounded text-[9px] font-black uppercase ${t.type === 'given' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'}`}>
                        {t.type === 'given' ? 'Given' : 'Received'}
                      </span>
                    </td>
                    <td className="px-4 py-2">{t.description || '-'}</td>
                    <td className={`px-4 py-2 font-black text-right ${t.type === 'given' ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
              ...
              {transactions.length > 0 && (
                <tfoot className="bg-slate-50 dark:bg-slate-800 border-t dark:border-slate-700 font-black">
                  <tr>
                    <td colSpan="4" className="px-4 py-3 text-right text-slate-700 dark:text-slate-300 uppercase">
                      Period Totals
                    </td>
        ...
        <div className="flex justify-between items-center bg-white dark:bg-slate-800 px-4 py-3 border dark:border-slate-700 shadow-sm">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="p-1 border border-slate-300 dark:border-slate-600 disabled:opacity-30 hover:bg-slate-50 dark:hover:bg-slate-700 rounded"
            >
