...
import { Plus, Trash2, Edit2, FileSpreadsheet, Printer, RotateCcw, Search, X } from "lucide-react";
...
const OtherTransactionsList = ({ showNotification }) => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html><head><title>Other Transactions</title>
      <style>
      ...
      <tbody>${transactions.map(t => `<tr><td>${new Date(t.date).toLocaleDateString('en-GB')}</td><td>${t.type === 'income' ? 'Income' : 'Expense'}</td><td>${t.description || '-'}</td><td>${t.amount}</td></tr>`).join('')}</tbody>
      </table>
      </body></html>
    `);
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {transactions.map(t => (
                  <tr key={t._id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 ${selectedIds.includes(t._id) ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''}`}>
                    ...
                    <td className={`px-4 py-2 font-black text-right ${t.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>₹{t.amount.toLocaleString('en-IN')}</td>
                ...
                {transactions.length === 0 && <tr><td colSpan="6" className="p-8 text-center text-slate-400 italic">No transactions found</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
